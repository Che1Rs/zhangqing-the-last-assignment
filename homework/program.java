...
public class program {
    public static void main(String[] args){
        ...
        for(int i=6;i<9;i++)
            totalSalary += employees[i].monthlyEarning();
        System.out.println("2. Total monthly salary of all the part-time staff: " + totalSalary);

        // c
        totalSalary = 0;
        for(int i= 0;i<9;i++)
            totalSalary += employees[i].monthlyEarning();
        System.out.println("3. Total monthly salary of all employees: " + totalSalary);

        // d
        System.out.println("4. Display all employee by id descending.");
        Arrays.sort(employees);
        for(int i=0;i<9;i++)
            System.out.println(employees[i].toString());

        // e
        System.out.println("5. Display all employee by last name ascending.");
        Arrays.sort(employees, new EmployeeLastNameComparator());
        for(int i=0;i<9;i++)
            System.out.println(employees[i].toString());

        // f
        System.out.println("6. Clone a faculty");
